...
import { Settings, Play, Plus, Activity, GitBranch, Binary, Network, TrendingUp, Info, CheckCircle2, ArrowRight, HelpCircle, AlertCircle, Zap, Trash2 } from 'lucide-react';
...
const ModelSelection = ({ isDarkMode, onNext, onPrev, dataset, datasetSchema, targetColumn, setTrainedModelResult, domain }) => {
    ...
    // M3: wrapped in useCallback with all deps — fixes stale-closure in autoRetrain effect
    const trainModel = useCallback(async () => {
        // H4: abort any in-flight request before launching a new one
        ...
        try {
            const metrics = await runMLTraining(
                ...
                datasetSchema,
                targetColumn,
                abortRef.current.signal, // H4: pass signal for cancellation
            );
    ...
    }, [selectedModel, params, dataset, datasetSchema, targetColumn, isInitialLoading, setTrainedModelResult]);
                    ...
                    <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
                        <div className={`px-6 py-4 border-b flex justify-between items-center ${isDarkMode ? 'border-slate-700' : 'border-slate-100'}`}>
                            ...
                            <div className="flex items-center gap-3">
                                {comparisonList.length === 0 && <span className="text-xs text-slate-500 italic hidden sm:inline">Train and click "+ Compare" to add results</span>}
                            ...
                            <table className="w-full text-sm text-left">
                                <thead className={`${isDarkMode ? 'text-slate-400 bg-slate-800' : 'text-slate-500 bg-slate-50'}`}>
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 font-semibold">F1</th>
                                    ...
                                    {comparisonList.length > 0 ? comparisonList.map((res) => {
                                        ...
                                        return (
                                            <tr key={res.id} className={`${isDarkMode ? 'hover:bg-slate-800/80' : 'hover:bg-slate-50/80'} transition-colors`}>
                                                ...
                                                <td className={`px-6 py-4 font-bold ${metricColor(res.accuracy)}`}>
                                                    {res.accuracy}
                                                </td>
                                                <td className={`px-6 py-4 font-bold ${sensColor} ${sensBg} rounded`}>
                                                    {res.sensitivity}
                                                    {sensVal < 0.50 && <span className="ml-1 text-[9px]">🚨</span>}
                                                </td>
                                                <td className={`px-6 py-4 font-semibold ${metricColor(res.specificity)}`}>{res.specificity}</td>
                                                <td className={`px-6 py-4 font-semibold ${metricColor(res.precision || '0')}`}>{res.precision || '—'}</td>
                                                <td className={`px-6 py-4 font-semibold ${metricColor(res.f1Score || '0')}`}>{res.f1Score || '—'}</td>
                                                <td className={`px-6 py-4 font-semibold ${metricColor(res.auc)}`}>{res.auc}</td>
                                            </tr>
                                        );
                                    }) : (
                                        <tr>
                                            <td colSpan="7" className="px-6 py-8 text-center opacity-50">
                                                No models compared yet. Train a model and click "+ Compare" to add rows.
                                            </td>
                                        </tr>
                                    )}
                                    {/* Live Result Preview Row */}
                                    {lastResult && !comparisonList.find(r => r.modelName === lastResult.modelName && r.settings === lastResult.settings) && (
                                        <tr className={`${isDarkMode ? 'bg-indigo-900/10' : 'bg-indigo-50'} border-l-4 border-indigo-500`}>
                                            ...
                                            <td className={`px-6 py-4 font-bold ${parseFloat(lastResult.sensitivity) >= 0.80 ? 'text-emerald-500' : parseFloat(lastResult.sensitivity) >= 0.60 ? 'text-amber-500' : 'text-rose-500'}`}>{lastResult.sensitivity}</td>
                                            <td className="px-6 py-4">{lastResult.specificity}</td>
                                            <td className="px-6 py-4">{lastResult.precision || '—'}</td>
                                            <td className="px-6 py-4">{lastResult.f1Score || '—'}</td>
                    ...
                    {comparisonList.length > 0 && (
                        <div className={`rounded-xl border overflow-hidden relative shadow-lg ${isDarkMode ? 'bg-slate-900/80 border-slate-700/50 backdrop-blur-xl' : 'bg-white border-slate-200'}`}>
                            ...
                            <div className="p-6 space-y-6 relative z-10">
                                {['accuracy', 'sensitivity', 'specificity', 'precision', 'f1Score', 'auc'].map(metric => {
                                    const labels = { accuracy: '🎯 Accuracy', sensitivity: '⭐ Sensitivity', specificity: '🛡 Specificity', precision: '🔬 Precision', f1Score: '⚖️ F1 Score', auc: '📈 AUC-ROC' };
                                    const thresholds = { accuracy: 0.65, sensitivity: 0.70, specificity: 0.65, precision: 0.60, f1Score: 0.65, auc: 0.75 };
