    ...
    return (
        <div>
            <p style={{ fontSize: '12px', color: COLORS.muted, marginBottom: '10px' }}>
                ...
                <span style={{ color: COLORS.green, fontWeight: 600 }}>green dashed circle</span> shows the{' '}
            ...
            <div style={legendStyle}>
                ...
                <div style={legendItemStyle}><div style={{ width: 20, height: 2, background: COLORS.green, borderRadius: '999px', border: '1px dashed #fff' }} /> K-radius</div>
            </div>
            <div style={{...bannerStyle, background: `${secondaryStr}15`, borderColor: secondaryStr, color: secondaryStr}}>
                ...
                <div><b style={{color: secondaryStr}}>Clinical meaning:</b> Of the {k} most similar patients (by {fNames[0].toLowerCase()} and {fNames[1].toLowerCase()}), {neighborInfo.readmitted} were {tName} and {neighborInfo.safe} were not. The model predicts: {neighborInfo.pct >= 50 ? tName : 'safe'} ({neighborInfo.pct}% confidence).</div>
            </div>
        </div>
    );
});
...
const SVMViz = React.memo(({ params, datasetSchema, targetColumn, primaryStr, secondaryStr }) => {
    ...
    return (
        <div>
            ...
            <div style={{ ...bannerStyle, background: `${secondaryStr}15`, borderColor: secondaryStr, color: secondaryStr }}>
                <span style={{ fontSize: 16, flex: '0 0 auto', marginTop: 1 }}>✅</span>
                <div><b style={{color: secondaryStr}}>Clinical meaning:</b> The thick line separates low-risk from high-risk patients. Rather than memorizing every patient, SVM only cares about the trickiest patients (support vectors) who define exactly where the safe zone ends.</div>
            </div>
        </div>
    );
});
...
const RFViz = React.memo(({ params, datasetSchema, targetColumn, primaryStr, secondaryStr }) => {
    ...
    return (
        <div>
            ...
            <div style={{ ...bannerStyle, background: `${secondaryStr}15`, borderColor: secondaryStr, color: secondaryStr }}>
                <span style={{ fontSize: 16, flex: '0 0 auto', marginTop: 1 }}>✅</span>
                <div><b style={{color: secondaryStr}}>Clinical meaning:</b> {readmitVotes} out of {n} trees predict {tName.toLowerCase()}. Final prediction: {tName.toUpperCase()} ({readmitPct}% confidence). This is like getting {n} second opinions — more stable than a single tree.</div>
            </div>
        </div>
    );
});
...
const LRViz = React.memo(({ params, datasetSchema, targetColumn, primaryStr, secondaryStr }) => {
    ...
    return (
        <div>
            ...
            <div style={{ ...bannerStyle, background: `${secondaryStr}15`, borderColor: secondaryStr, color: secondaryStr }}>
                <span style={{ fontSize: 16, flex: '0 0 auto', marginTop: 1 }}>✅</span>
                <div><b style={{color: secondaryStr}}>Clinical meaning:</b> Patient with {fNames[0]}=38 sits at the steep part of the curve — small changes in this value cause large swings in risk. This is the &quot;tipping point&quot; where intervention matters most.</div>
            </div>
        </div>
    );
});
...
const NBViz = React.memo(({ datasetSchema, targetColumn, primaryStr, secondaryStr }) => {
    ...
    return (
        <div>
            ...
            <div style={{ ...bannerStyle, background: `${secondaryStr}15`, borderColor: secondaryStr, color: secondaryStr }}>
                <span style={{ fontSize: 16, flex: '0 0 auto', marginTop: 1 }}>✅</span>
                <div><b style={{color: secondaryStr}}>Clinical meaning:</b> The model treats each measurement independently (&apos;naïve&apos; assumption). In reality, age and kidney function are related, but this simple approach often works well in practice.</div>
            </div>
        </div>
    );
});
